...
extern "C" {
  ...
  SEXP BRISC_predict(SEXP X_r, SEXP y_r, SEXP coords_r, SEXP n_r, SEXP p_r, SEXP m_r, SEXP X0_r, SEXP coords0_r, SEXP q_r, SEXP nnIndx0_r,
...
#pragma omp parallel for private(threadID, phi, nu, sigmaSq, tauSq, k, l, d, info)
	...
	d = 0;
	for(k = 0; k < m; k++){
	  d += tmp_m[threadID*m+k]*(y[nnIndx0[i+q*k]] - F77_NAME(ddot)(&p, &X[nnIndx0[i+q*k]], &n, &beta[p], &inc));
	}
	y0[i] = F77_NAME(ddot)(&p, &X0[i], &q, &beta[p], &inc) + d;
