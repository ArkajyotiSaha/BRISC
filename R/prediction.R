BRISC_prediction <- function(BRISC_Out, X.0, coords.0, n_omp = 1, verbose = FALSE){
  ...
  out <- .Call("BRISC_predict", X, y, coords, n, p, n.neighbors, X.0, coords.0, q, nn.indx.0,
               Beta, Theta, cov.model.indx, n.omp.threads, verbose, n.report)
  p6 <- proc.time()
